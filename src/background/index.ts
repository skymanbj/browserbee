...
function setupEventListeners(): void {
      ...
      memoryService
        ...
        .then(async () => {
          ...
          if (details.reason === "install") {
            try {
              logWithTimestamp(
                "Importing default memories for new installation",
              );
              const importedCount = await memoryService.importDefaultMemories();
              if (importedCount > 0) {
                logWithTimestamp(
                  `Successfully imported ${importedCount} default memories`,
                );
              } else {
              ...
              logWithTimestamp(
                `Error importing default memories: ${error}`,
                "error",
              );
            }
          }
        ...
        .catch((error) => {
          logWithTimestamp(
            `Error initializing memory database: ${error}`,
            "error",
          );
        });
    }
  ...
  chrome.action.onClicked.addListener(async (tab) => {
    if (tab.id) {
      logWithTimestamp(`Opening custom sidebar for tab ${tab.id}`);

      try {
        // First try: send message to content script
        await chrome.tabs.sendMessage(tab.id, { action: 'toggleSidebar' });
        logWithTimestamp(`Toggle sidebar message sent to tab ${tab.id}`);
      } catch (error) {
        const errorStr = String(error);
        
        // Content script not loaded - use fallback immediately
        if (errorStr.includes('Receiving end does not exist') || 
            errorStr.includes('Could not establish connection')) {
          logWithTimestamp(`Content script not ready, trying native side panel: ${errorStr}`, 'warn');
          
          try {
            await chrome.sidePanel.open({ tabId: tab.id });
            logWithTimestamp(`Native side panel opened for tab ${tab.id}`);
          } catch (fallbackError) {
            logWithTimestamp(`Fallback also failed: ${String(fallbackError)}`, 'error');
          }
        } else {
          logWithTimestamp(`Error sending toggle message: ${errorStr}`, 'error');
        }
      }
    } else {
    ...
    try {
      ...
      try {
        ...
        request.onerror = (event) => {
          logWithTimestamp(
            `Error deleting memory database: ${(event.target as IDBRequest).error}`,
            "error",
          );
        };
      } catch (error) {
        logWithTimestamp(
          `Exception deleting memory database: ${error}`,
          "error",
        );
      }
    } catch (error) {
  ...
  chrome.runtime.onUpdateAvailable.addListener(async (details) => {
    logWithTimestamp(
      `Extension update available: ${details.version}, cleaning up resources`,
    );
    try {
      await cleanupOnUnload();
      ...
      try {
        ...
        request.onsuccess = () => {
          logWithTimestamp(
            "Memory database deleted successfully before update",
          );
        };

        request.onerror = (event) => {
          logWithTimestamp(
            `Error deleting memory database before update: ${(event.target as IDBRequest).error}`,
            "error",
          );
        };
      } catch (error) {
        logWithTimestamp(
          `Exception deleting memory database before update: ${error}`,
          "error",
        );
      }
    } catch (error) {
      logWithTimestamp(
        `Error during pre-update cleanup: ${String(error)}`,
        "error",
      );
    }
  });
  ...
  try {
      ...
      chrome.sidePanel.onShown.addListener(async (info: { tabId?: number }) => {
        ...
        if (info.tabId) {
          ...
          try {
            ...
            logWithTimestamp(
              `Side panel shown for tab ${info.tabId} in window ${windowId}`,
            );
            ...
            logWithTimestamp(
              `Error getting window ID for tab ${info.tabId}: ${String(error)}`,
              "error",
            );
    ...
    logWithTimestamp(
      "Side panel events not available: " + String(error),
      "warn",
    );
...
function setupCommandListeners(): void {
  ...
  chrome.commands
    ...
    .catch((error) => {
      logWithTimestamp(
        `Error getting registered commands: ${String(error)}`,
        "error",
      );
    });
