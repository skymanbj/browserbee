...
export class ScheduledTaskService {
  ...
  /** 获取所有任务 */
  public getAll(): ScheduledTask[] {
    return Array.from(this.tasks.values())
      .sort((a, b) => b.createdAt - a.createdAt);
  }

  /** 获取已启用的任务 */
  public getEnabled(): ScheduledTask[] {
    return this.getAll().filter(t => t.enabled && t.status === 'active');
  }

  /** 根据 ID 获取任务 */
  public getById(id: string): ScheduledTask | undefined {
  ...
  public async exportToJson(): Promise<string> {
    const tasks = this.getAll().filter(t => !t.isBuiltIn);
  ...
  /** 获取任务统计 */
  public getStats(): { total: number; active: number; paused: number; builtIn: number } {
