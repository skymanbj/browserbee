...
export const useChromeMessaging = ({
  ...
  useEffect(() => {
    const messageListener = (
      ...
      if (
        (message.tabId && message.tabId !== tabId) ||
        (message.windowId && windowId && message.windowId !== windowId)
      ) {
        return false; // Skip messages for other tabs or windows
      }

      // Prevent invalidated-context noise when sidepanel is tearing down.
      // If the sender closes before sendResponse, a try/catch helps avoid additional console noise.
      const safeSendResponse = (resp: any) => {
        try {
          sendResponse(resp);
        } catch (e) {
          // ignore
      ...
      if (message.action === "updateOutput") {
        ...
        if (
          ...
          const titleMatch = message.content.content.match(
            /Connected to tab: (.+)/,
          );
        ...
        onFallbackStarted(
          message.content?.message ||
            "Switching to fallback mode. Processing continues...",
        );
        ...
        if (
          ...
          if (onRequestApproval) {
            ...
            console.error(
              "onRequestApproval handler is not defined, cannot process approval request",
            );
          }
          ...
          safeSendResponse({
            success: false,
            error: "Incomplete approval request",
          });
        }
      } else if (
        ...
        message.action === "activeTabChanged" &&
        message.oldTabId &&
        message.newTabId
      ) {
        ...
        console.log(
          `Active tab changed from ${message.oldTabId} to ${message.newTabId}`,
        );
  ...
  const approveRequest = (requestId: string) => {
    chrome.runtime.sendMessage(
      ...
      (_response) => {
        if (chrome.runtime.lastError) {
          console.error(
            "Error sending approval response:",
  ...
  const rejectRequest = (requestId: string) => {
    chrome.runtime.sendMessage(
      ...
      (_response) => {
        if (chrome.runtime.lastError) {
          console.error(
            "Error sending rejection response:",
            chrome.runtime.lastError,
          );
        }
      },
